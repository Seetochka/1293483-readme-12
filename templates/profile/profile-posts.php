        ...
        <article class="profile__post post post-<?= $post['class_name']; ?>">
                        ...
                        <a class="post__author-link" href="/profile.php?id=<?= $post['author_id']; ?>" title="Автор">
                            ...
                            <div class="post__info">
                                ...
                                <time class="post__time" datetime="<?= $post['original_post_dt_add']; ?>"
                                      title="<?= date_format(date_create($post['original_post_dt_add']),
                                          'd.m.Y H:i'); ?>"><?= format_time($post['original_post_dt_add']); ?> назад
                                </time>
                            </div>
                        </a>
                    </div>
                ...
                <?php if (!in_array($post['id'], $showed_comments_post_ids)): ?>
                    <div class="comments">
                        <a class="comments__button button" href="<?= get_query_href([
                            'comments' => !$showed_comments_post_ids ? $post['id'] : filter_input(INPUT_GET,
                                    'comments') . "+{$post['id']}"
                ...
                <?php else: ?>
                    <div class="comments__list-wrapper">
                        <ul class="comments__list">
                            <?php for ($i = 0, $comment_num = min(count($post['comments']),
                                $post['comments_count_in_page']); $i < $comment_num; $i++): ?>
                                <li class="comments__item user">
                                    <div class="comments__avatar">
                                        <a class="user__avatar-link"
                                           href="../profile.php?id=<?= $post['comments'][$i]['user_id']; ?>">
                                            <img class="comments__picture"
                                                 src="../<?= $post['comments'][$i]['avatar'] ?? 'img/icon-input-user.svg'; ?>"
                                    ...
                                    <div class="comments__info">
                                        <div class="comments__name-wrapper">
                                            ...
                                            <time class="comments__time"
                                                  ...
                                                  title="<?= date_format(date_create($post['comments'][$i]['dt_add']),
                                                      'd.m.Y H:i'); ?>"><?= format_time($post['comments'][$i]['dt_add']); ?>
                                                назад
                                            </time>
                                        </div>
                        ...
                        <?php if ($post['comments_count'] > MAX_COMMENT_COUNT && $post['comments_count'] !== $post['comments_count_in_page']): ?>
                            <a class="comments__more-link" href="<?= get_query_href([
                                'comments-all' => !$showed_comments_all_post_ids ? $post['id'] : filter_input(INPUT_GET,
                                        'comments-all') . "+{$post['id']}"
                            ], '/profile.php') ?>">
            ...
            <?php if ($post['comments_count'] === 0 || in_array($post['id'], $showed_comments_post_ids)): ?>
                <form class="comments__form form" method="post" style="margin-bottom: 0">
                    ...
                    <div class="form__input-section <?= !empty($errors) && !empty($new_comment['post_id']) && (int)$new_comment['post_id'] === $post['id'] ? 'form__input-section--error' : ''; ?>">
                        <input type="hidden" name="post_id" value="<?= $post['id']; ?>">
                        <textarea class="comments__textarea form__textarea" placeholder="Ваш комментарий"
                                  name="content"><?= !empty($new_comment['post_id']) && (int)$new_comment['post_id'] === $post['id'] ? htmlspecialchars($new_comment['content']) : ''; ?></textarea>
