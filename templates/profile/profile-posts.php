        ...
        <article class="profile__post post post-<?= $post['class_name']; ?>">
                    ...
                    <div class="post__author">
                        <a class="post__author-link" href="/profile.php?id=<?= $post['author_id']; ?>" title="Автор">
                            <div class="post__avatar-wrapper post__avatar-wrapper--repost">
                                <img class="post__author-avatar" src="<?= $post['author_avatar'] ?? 'img/icon-input-user.svg'; ?>" alt="Аватар пользователя" width="60" height="60">
                            </div>
                            <div class="post__info">
                                <b class="post__author-name">Репост: <?= htmlspecialchars($post['author_login']); ?></b>
                                <time class="post__time" datetime="<?= $post['original_post_dt_add']; ?>"><?= format_time($post['original_post_dt_add']); ?> назад</time>
                            </div>
                        </a>
                    </div>
                <?php endif; ?>
                <h2 style="padding: 29px 40px;"><a href="../post.php?id=<?= $post['id']; ?>"><?= htmlspecialchars($post['title']); ?></a></h2>
            </header>
                ...
                <?php switch ($post['class_name']):
                    ...
                    <?php case 'photo': ?>
                        <div class="post-photo__image-wrapper">
                            <img src="<?= htmlspecialchars($post['photo']); ?>" alt="<?= htmlspecialchars($post['title']); ?>" width="760" height="396">
                        </div>
                        <?php break; ?>
                    <?php case 'video': ?>
            ...
            <footer class="post__footer">
                <div class="post__indicators">
                    <div class="post__buttons">
                        ...
                        <a class="post__indicator post__indicator--repost button" <?php if ($post['user_id'] !== $user_data['id']): ?>href="/actions/repost.php?id=<?= $post['id']; ?>"<?php endif; ?> title="Репост">
                    ...
                    <time class="post__time" datetime="<?= $post['dt_add']; ?>"><?= format_time($post['dt_add']); ?>назад</time>
                </div>
                <ul class="post__tags">
            ...
            <?php if ($post['comments_count'] > 0): ?>
                <?php if (!in_array($post['id'], $showed_comments_post_ids)): ?>
                    <div class="comments">
                        <a class="comments__button button" href="<?= get_query_href(['comments' => !$showed_comments_post_ids ? $post['id'] : filter_input(INPUT_GET, 'comments') . "+{$post['id']}"], '/profile.php'); ?>">Показать комментарии</a>
                    </div>
                <?php else: ?>
            ...
            <?php if ($post['comments_count'] === 0 || in_array($post['id'], $showed_comments_post_ids)): ?>
                <form class="comments__form form" method="post" style="margin-bottom: 0">
                    <div class="comments__my-avatar">
                        <img class="comments__picture" src="../<?= $user_data['avatar'] ?? 'img/icon-input-user.svg'; ?>" alt="Аватар пользователя" width="40" height="40">
                    </div>
                    <input type="hidden" name="post_id" value="<?= $post['id']; ?>">
                    <textarea class="comments__textarea form__textarea" placeholder="Ваш комментарий" name="content"><?= $new_comment['post_id'] === $post['id'] ? htmlspecialchars($new_comment['content']) : ''; ?></textarea>
