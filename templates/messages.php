            ...
            <ul class="messages__contacts-list tabs__list">
                <?php foreach ($interlocutors as $interlocutor) : ?>
                    <li class="messages__contacts-item">
                        ...
                        <?= $interlocutor['id'] === $interlocutor_id ? 'messages__contacts-tab--active' : ''; ?>
                        tabs__item
                        <?= $interlocutor['id'] === $interlocutor_id ? 'tabs__item--active' : ''; ?>"
                           href="<?= get_query_href(['id' => $interlocutor['id']], '/messages.php'); ?>">
                            <div class="messages__avatar-wrapper">
                                ...
                                <?php if (!empty($interlocutor['unread_messages_count']) &&
                                    $interlocutor['unread_messages_count'] > 0) : ?>
                            ...
                            <div class="messages__info">
                                ...
                                <div class="messages__preview">
                                    <p class="messages__preview-text">
                                        <?= !empty($interlocutor['content']) ?
                                            htmlspecialchars($interlocutor['content']) : ''; ?>
                                    </p>
                                    <time class="messages__preview-time"
                                          datetime="<?= !empty($interlocutor['dt_add']) ?
                                              $interlocutor['dt_add'] : ''; ?>"
                                          title="<?= !empty($interlocutor['dt_add']) ?
                                              date_format(date_create($interlocutor['dt_add']), 'd.m.Y H:i') : ''; ?>">
                    ...
                    <?php if (!empty($messages) && count($messages) > 0) : ?>
                        <ul class="messages__list tabs__content tabs__content--active">
                            <?php foreach ($messages as $message) : ?>
                                <li class="messages__item <?= $message['sender_id'] === $user_data['id'] ?
                                    'messages__item--my' : ''; ?>">
                                    <div class="messages__info-wrapper">
                                        ...
                                        <div class="messages__item-info">
                                            ...
                                            <time class="messages__time" datetime="<?= $message['dt_add']; ?>"
                                                  title="
                                                  <?= date_format(date_create($message['dt_add']), 'd.m.Y H:i'); ?>">
                    ...
                    <form class="comments__form form" method="post">
                        ...
                        <div class="form__input-section <?= !empty($errors) ? 'form__input-section--error' : ''; ?>">
                            <textarea class="comments__textarea form__textarea form__input" placeholder="Ваше сообщение"
                                      name="content"><?= !empty($new_message['content']) ?
                                        htmlspecialchars($new_message['content']) : ''; ?></textarea>
